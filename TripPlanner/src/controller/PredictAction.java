...
public class PredictAction extends Action {
	...
	public String perform(HttpServletRequest request) {
		...
		if ("true".equals(getAllRoute)) {
			...
			double lon = Double.parseDouble(((String) vehicle.get("lon")).trim());
			double lat = Double.parseDouble(((String) vehicle.get("lat")).trim());

			Prediction prediction = new Prediction(lat, lon, stopId, curStop.getStopName(), route, direction,
					(String) jPredict.get("vid"), String.valueOf(gapTime), predTime);
